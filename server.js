...
app.use((req, res, next) => {
    res.header('Access-Control-Allow-Origin', '*'); // 替换为允许的客户端域名
...
//      let link = 'http://tapshark.co/tapShark/index.html?'+'userId='+cryptoUtilsHelper.encrypt(rows[i].email)+'cardNum='+cryptoUtilsHelper.encrypt(rows[i].card_num);
//	let link = 'http://tapshark.co/tapShark/index.html?'+'userId='+cryptoUtilsHelper.encrypt(rows[i].email)+'cardNum='+cryptoUtilsHelper.encrypt(''+rows[i].card_num);
	    cardLinks.push({link});
    }
    return cardLinks;
...
app.post('/cardmanager/cardDetailByOrderIdUpdate', function (req, res) {

    var orderId = req.body.orderId;


    orderDetailUtils.cardDetailByOrderIdUpdateState(orderId,function(status,rows){
        var obj = new response()
        obj.code = status;
...
var port = normalizePort(process.env.PORT_S || '3000');
